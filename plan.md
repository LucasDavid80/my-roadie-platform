...
- ✅ **Lembretes Locais de Eventos Próximos (Mobile):** Implementado `NotificationService` como singleton encapsulando `flutter_local_notifications` e `timezone`, lidando com agendamento local absoluto (`zonedSchedule`). O agendamento ocorre via chamadas ao serviço durante a criação/edição/exclusão no `AgendaController`. Para Android, foi criado o canal de alta prioridade e ativada a permissão `SCHEDULE_EXACT_ALARM`, exigindo ativação de `core library desugaring` para conversão de datas; para iOS solicita-se permissão em runtime (Spec 023).
