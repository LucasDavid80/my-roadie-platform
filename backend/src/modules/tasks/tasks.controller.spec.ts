...
describe('TasksController', () => {
  ...
  describe('findAll', () => {
    ...
    it('deve passar o eventId ao service quando fornecido via query', async () => {
      const result = await controller.findAll('event-uuid-123');

      expect(service.findAll).toHaveBeenCalledWith('event-uuid-123');
      expect(result).toEqual([mockTask]);
    });
  });
  ...
  describe('findOne', () => {
    it('deve retornar a tarefa pelo ID chamando o service', async () => {
      const result = await controller.findOne('task-uuid-123');

      expect(service.findOne).toHaveBeenCalledWith('task-uuid-123');
      expect(result).toEqual(mockTask);
    });
  ...
  describe('update', () => {
    it('deve atualizar a tarefa chamando o service com ID e DTO', async () => {
      ...
      const result = await controller.update('task-uuid-123', dto);

      expect(service.update).toHaveBeenCalledWith('task-uuid-123', dto);
    ...
    it('deve repassar NotFoundException se a tarefa a ser atualizada não for encontrada', async () => {
      ...
      await expect(
        controller.update('id-inexistente', { isDone: true }),
      ).rejects.toThrow(NotFoundException);
    });
  });

  describe('remove', () => {
    it('deve remover a tarefa chamando o service com o ID', async () => {
      const result = await controller.remove('task-uuid-123');

      expect(service.remove).toHaveBeenCalledWith('task-uuid-123');
      expect(result).toEqual(mockTask);
    });
