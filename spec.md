...
- ✅ **events** — CRUD completo de eventos (`POST /events`, `GET /events`, `GET /events/:id`, `PATCH /events/:id`, `DELETE /events/:id`), com DTOs validados via `class-validator` (`CreateEventDto`, `UpdateEventDto`), autenticação via `JwtAuthGuard` vinculando `createdById` e `bandId` via Prisma, e cobertura de testes unitários e E2E (Spec 014).
