...
- ✅ **Histórico de Compromissos e Filtragem de Próximos Eventos:** Separação estrutural de compromissos na Agenda mobile; `AgendaController` disponibiliza os getters `upcomingEvents` (eventos com data `>= hoje`, ordenados cronologicamente) e `pastEvents` (eventos com data `< hoje`, ordenados do mais recente ao mais antigo) baseados na normalização de dia `_startOfDay`; `CommitmentsWidget` na `PrincipalScreen` exibe exclusivamente próximos compromissos e disponibiliza atalho "Ver histórico" navegando via `GoRouter` para a rota `/history`; nova tela dedicada `HistoryScreen` (`presentation/screens/history/history_screen.dart`) lista todos os compromissos passados agrupados por data com estado vazio amigável e ações completas de edição e exclusão com confirmação via `CommitmentCard`; `InfosWidget` atualizado para derivar `compromissosTotal` de `upcomingEvents.length`; cobertura completa de testes unitários (`agenda_controller_test.dart`) e de widget (`history_screen_test.dart`, `principal_screen_test.dart`) (Spec 021).
