...
- ✅ **Pipeline CI/CD Unificado & Modernizado:** Workflow completo no GitHub Actions com detecção de alterações por diretório via `dorny/paths-filter@v3` (filtrando `backend`, `frontend`, `mobile` e economizando minutos de CI), disparo manual `workflow_dispatch` com parâmetros (`scope`, `backend_url`, `run_mobile_e2e`), testes E2E automatizados do backend (`npm run test:e2e` via Jest e mocks de JWKS) e do frontend web (Playwright com browser Chromium headless), gancho preparatório de E2E Mobile sob demanda para a Spec 018, compilação e publicação simétrica de artefatos de release (.apk Android e .ipa iOS empacotado em runner macOS) com `--dart-define=BACKEND_URL` e retenção de 7 dias, caching multi-camadas (npm, Flutter pub e Gradle), e Continuous Deployment automatizado via webhooks para Render (backend) e Vercel (frontend) condicionados ao sucesso dos builds na branch `main` (Spec 017).
...
Esta spec serve como referência congelada. Ela é considerada válida enquanto bater com o código — atualizada após a conclusão do Pipeline CI/CD Unificado & Modernizado (Spec 017) e o fechamento de todos os itens priorizados da Fase 1 do backlog. A entrega desta fase incluiu estabilização completa do ecossistema, segurança JWT/CORS, release do MVP para testes fechados, suítes de testes unitários e E2E, caching multi-camadas, e automação de Continuous Deployment. Se qualquer item da seção 4 mudar nas próximas specs, este arquivo deve ser atualizado na spec correspondente.
