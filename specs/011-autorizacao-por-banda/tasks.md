...
- [x] T0.3 — Confirmar se existe `@CurrentUser()` (ou equivalente) já usado em `users`/`events`; se não existir, criar um só, reaproveitado pelos três módulos.
...
- [x] T2.3 — Testes: 3 positivos + 3 negativos cobrindo os 4 cenários do critério de sucesso da spec.

## Fase 3 — Aplicar em Transactions

- [x] T3.1 — Mesmo padrão da Fase 2, preservando os filtros existentes (`userId`, `eventId`, `type`).
- [x] T3.2 — Testes: 3 positivos + 3 negativos.
