...
- [ ] T6.3 — Configurar job `mobile-ios-build` com runner `macos-latest` no pipeline de CI/CD (`.github/workflows/ci.yml`) para compilar o iOS em release com `--dart-define=BACKEND_URL=<url-producao>`.
- [ ] T6.4 — Configurar empacotamento do `Runner.app` em `my-roadie-release.ipa` e upload do artefato no GitHub Actions.
