...
> [!IMPORTANT]
> **Supersedido parcialmente pela Spec 024:** Os campos `startTime` e `endTime` referenciados nestas tasks foram substituídos por `startsAt`, `endsAt` e `timezone` na Spec 024. O conteúdo abaixo reflete o estado do projeto na época da execução.
...
- [x] T4.2 — Executar os testes automatizados do mobile (`flutter test` em `mobile/`).
- [x] T4.3 — Validar a integridade dos builds do backend, web e mobile (`npm run build` / `flutter analyze`).
