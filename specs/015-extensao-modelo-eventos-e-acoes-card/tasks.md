...
- [ ] T1.1 — Atualizar o modelo `Event` e a relação com `Transaction` em `backend/prisma/schema.prisma` com `startTime`, `endTime`, `type`, `fee` e `onDelete: Cascade`.
- [ ] T1.2 — Executar `npx prisma generate` no diretório `backend/` para atualizar o client do Prisma.
...
- [ ] T2.1 — Atualizar `CreateEventDto` e `UpdateEventDto` com validações `class-validator` para `startTime`, `endTime`, `type` e `fee`.
- [ ] T2.2 — Atualizar `backend/src/modules/events/entities/event.entity.ts` com as novas propriedades.
...
- [ ] T3.1 — Atualizar `EventModel.toCreatePayload()` e `EventModel.fromMap()` para serializar e desserializar `startTime`, `endTime`, `type` e `fee`.
- [ ] T3.2 — Atualizar `CommitmentCard` substituindo o ícone estático por um `IconButton` funcional com modal de confirmação para exclusão de compromisso.
- [ ] T3.3 — Conectar o callback de exclusão em `CommitmentsWidget` e `PrincipalScreen` chamando `AgendaController.deleteEvent(id)` com feedback visual (`SnackBar`).
- [ ] T3.4 — Ajustar o layout horizontal, a renderização dinâmica do campo Cachê (exibido apenas para `Show` e `Gravação`) e o alinhamento centralizado do botão de submissão em `NewAppointmentWidget`.
