...
- [ ] T1.1 — Implementar `_isSameMonth`, `monthlyEvents`, `monthlyFee` e ajustar `monthlyShows` com filtro estrito de tipo no `agenda_controller.dart`.
...
- [ ] T2.1 — Implementar formatação monetária padrão brasileiro (`R$ X.XXX,XX`) no campo de faturamento do `InfosWidget`.
- [ ] T2.2 — Adicionar ajuste responsivo com `FittedBox` no `infoCard` para evitar overflow e quebras de linha defeituosas com números grandes.
...
- [ ] T3.1 — Atualizar a invocação do `InfosWidget` na `PrincipalScreen` para consumir os getters corrigidos do `AgendaController`.
