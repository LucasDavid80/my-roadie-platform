...
2. **Disparo Manual Sob Demanda (`workflow_dispatch`)**: Permitir que desenvolvedores acionem builds de release avulsos (APK Android e IPA iOS) ou executem o pipeline sob demanda pelo painel do GitHub Actions com parâmetros configuráveis.
3. **Completude de Testes Automatizados (E2E)**: Integrar a execução dos testes E2E do Backend (`npm run test:e2e`) e dos testes E2E de Frontend Web (Playwright com browser headless Chromium) antes dos estágios de build e deploy.
4. **Padronização de Artefatos de Release**: Publicação automatizada tanto do `.apk` (Android) quanto do `.ipa` (iOS) via `actions/upload-artifact@v4`, ambos apontando para o backend de produção via `--dart-define=BACKEND_URL`, com retenção de 7 dias.
5. **Automação de Continuous Deployment (CD)**: Preservar e consolidar os hooks de deploy do Render (backend) e Vercel (frontend) condicionados ao sucesso dos testes e compilações na branch `main`.
6. **Otimização de Performance e Caching**: Caching multi-camadas (npm para Node 22, Flutter pub cache e Gradle cache para Android SDK).
...
1. **Gatilhos & Detecção de Mudanças**:
   - Adicionar `workflow_dispatch` com input opcional para forçar execução total ou de apps específicos (`all`, `backend`, `frontend`, `mobile`).
...
2. **Estágio de Qualidade & Testes (Lints, Unitários & E2E)**:
   ...
   - **Mobile**: `mobile-lint` (`flutter analyze`) -> `mobile-test` (`flutter test`).
...
- Criação ou refatoração de testes de negócio existentes (esta spec foca estritamente na automação e integração da infraestrutura de CI/CD).

## Critérios de Sucesso

- [ ] Gatilho `workflow_dispatch` adicionado e operacional no `.github/workflows/ci.yml`.
